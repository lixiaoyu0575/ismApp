...
angular.module('app')
    .controller('SPMedicationCtrl',SPMedicationCtrl);
function SPMedicationCtrl($scope, $rootScope, Data, SPMedicationSharedDataService) {
    $scope.isMax = false;
    $scope.pIds = [
        945209345,
        390870343,
    ...
    $scope.search = function (pId,str) {
       ...
       // $scope.isGraphLoading_dia = true;
        ...
        Data.getSankeyMedData(pId).then(function (res) {
              if(res.Return == 1){
           ...
           // console.log(sankeyData);
            var records = {};
            records.data = getRecords(res.Data);
            ...
            // console.log(records.dim.namecn.group().all());
            var namecnSumGroup = records.dim.namecn.group().reduce(
                function reduceAdd(p,v) {
                    // console.log(p);
                    // console.log(v);
                    // console.log(records.dim.namecn.top(2));
                    // console.log(records.dim.namecn.group());
                    // console.log(records.dim.namecn.group().top(1));
                    // console.log(p);
                    // console.log(v);
                    ...
                    // else p["value"]++;
                    p["value"] = (p["value"]||0) + 1;
                    // p[v.hierarchy] = (p[v.hierarchy]||0) + 1;
                    // p["value"]++;
                    // console.log(p);
                    return p;
                },
            ...
            //console.log(namecnSumGroup.all());
...
////////////////////////魏老师，就是这段代码，可能有些问题，因为逻辑太多，我没有搞明白／／／／／／／／／／／／／／／／
            ...
            for(var i=0;i<treemapData.length;i++){
                ...
                // console.log(treemapData[i]);
                // treemapData[i]["subregion"] = treemapData[i].value.hierarchy;
                // treemapData[i].value = treemapData[i].value.value;
                // console.log(treemapData[i]);
            }
            ...
            SPMedicationSharedDataService.addData("sankeyData", sankeyData);
            SPMedicationSharedDataService.addData("records", records);
            SPMedicationSharedDataService.addData("treemapRec", treemapRec);
            SPMedicationSharedDataService.addData("hierarchyJson", hierarchyJson);
