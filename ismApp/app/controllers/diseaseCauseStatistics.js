...
function DiseaseCauseStatisticsController($scope,Data,$rootScope, $location, $http) {
    ...
    $scope.RenderBubbleChart = function(pId){
        ...
        //@chen si rui
/*        var req1=d3.json('http://59.110.52.133:8000/usdata/riskScore/?q=%7B%22Pid%22:'+pId+'%7D')
            .header("apikey",$rootScope.Authenapikey)
            .header("token",$rootScope.userToken);

        var req2=d3.json('http://59.110.52.133:8000/usdata/featureImportance/?q=%7B%22Pid%22:'+pId+'%7D')
            .header("apikey",$rootScope.Authenapikey)
            .header("token",$rootScope.userToken);*/
        ...
        if($scope.isMax==false) {
            var width=$(".widget-body").width()*0.6;
...
/////////////////////////////// render bubblec chart //////////////////////////////////////////
        $http.get('http://59.110.52.133:3010/api/transfer/?q={"path":"/usdata/riskScore","params":%7B%22Pid%22:'
                + pId + '%7D}').success(function (diseaseNew) {
            $http.get('http://59.110.52.133:3010/api/transfer/?q={"path":"/usdata/featureImportance","params":%7B%22Pid%22:'
                    + pId + '%7D}').success(function (featureNew) {
                // if (error) throw error;
                console.log(diseaseNew);
                ...
                console.log(diseaseNew.Results);
                //console.log(diseaseNew.Results[0].risk_score);
                //console.log(diseaseNew.Results[1]);
                console.log(featureNew.Results);
                ...
                //var MyNodes=MaxNode(diseaseNew.Results);
                console.log(diseaseNew.Results);
                ...
                //console.log(nodes[0].risk_score);
                ...
                //var SelectedNode=GetNode(nodes);
                var MaxDisease=nodes[NodesLength-1];
                ...
                var featureValues=[];
                for (var i=0;i<featureNew.Results[maxname].length-1;i++)
                ...
                {
                    for (var j=0;j<risks.length-1-i;j++){
                        if(risks[j]>risks[j+1])
                        {
                            var temp=risks[j];
                            risks[j]=risks[j+1];
                            risks[j+1]=temp;
                ...
                console.log("get featureNew err", err);
            });
        }).error(function (err) {
            console.log("get diseaseNew err", err);
        });
