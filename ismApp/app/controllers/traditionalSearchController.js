...
angular.module("app").controller("traditionalSearchController", ["traMedical","medicalKnowledgeModal","$scope","$rootScope","$state",function (
  traMedical,medicalKnowledgeModal,$scope,$rootScope,$state, $location) {
     ...
     $scope.showlevel4 = function (class1,class2,class3,class4,class5) {
            ...
            class4, "", $scope.vm.pageSize, $scope.vm.pageNumber).then(function (res) {
                ...
                if (res.Results !== "" && res.Results !== undefined) {
                    ...
                    $scope.vm.pp = parseInt($scope.vm.totalNumber / $scope.vm.pageSize - 0.00001) +1;
    ...
    $scope.pages = function (type) {
        if(type == 0) {
            ...
            if($scope.vm.pageNumber >= parseInt($scope.vm.totalNumber /$scope.vm.pageSize - 0.001)){
                $scope.vm.pageNumber = parseInt($scope.vm.totalNumber /$scope.vm.pageSize - 0.001);
            }else{
                $scope.vm.pageNumber = $scope.vm.pageNumber + 1;
            }
        }else if(type == 3){
