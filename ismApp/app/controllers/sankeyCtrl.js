      ...
      $scope.createData_new = function(){
         ...
         for(var i = 0,leni = $scope.patients;i < $scope.patients&&i<datas.length;i++){
                ...
                for(var j = 0, lenj = 2*hlen; j<lenj&&j<8; j++){
                    ...
                    if(linkIndex > (datas[i].Sequences.length - 1))
                          {
                              stv.push({"source": "_" + j, 
                                       "target": "_" + (j+1),
                                        "value": 1
                              ...
                              for(var ii = 0, lenii = curValues.length; ii < lenii; ii++){
                                  if ((curValues[ii] == "undefined")){
                                  stv.push({"source": "检验化验" + "_" + j, 
                                                  "target": "_" + (j+1),
                                                  "value": 1});
                                  }else{
                                  ...
                                  for(var jj = 0, lenjj = nextValues.length; jj < lenjj; jj++){
                                      if(curValues[ii] == "undefined"){
                                          stv.push({"source": "检验化验" + "_" + j, 
                                                    ...
                                                    "value": 1});
                                      }else if (nextValues[jj] == "undefined"){
                                          stv.push({"source": curValues[ii] + "_" + j, 
                                                    "target":  "检验化验" + "_" + (j+1),
                                                    "value": 1});
                                      }else {
      ...
      var link = svg1.append("g").selectAll(".link")
        ...
        .enter().append("path")
         ...
         .style("display", function(d,i){
            if(d.source.name.length == 2 || d.target.name.length == 2){
              return "none";
            }else{
              return "block";
            }
