...
(function () {
    ...
    /* @ngInject */
    function CPcomparisonController($timeout,$scope,$rootScope, $location) {
        ...
        cliPath.createCrossfiterGraphs = function (data) {//receive a dict here for now
            ...
            costChart.width(width)
                ...
                .on("postRender",function () {
                    ...
                    $("#cfCostBar .x-axis-label").css("transform","matrix(1, 0, 0, 1, "+ width/2 +", " + (height +　15) + ")");
        ...
        /*$.get("data/patients_info.json").success(function (res) {
            //http://59.110.52.133:9999/clinicalpath/queryPatients/
            // res = res.content;
            var patientsInfo = [];
            for (var k in res){
                res[k].Score/=5;
        ...
        function processData() {
            ...
            var queryItemsTimeP = $.ajax({
                url: 'http://59.110.52.133:3010/api/transfer?q={"path":"/clinicalpath/queryItemsTime","params":{"PID":"' +
                        cliPath.patientInfo.ID + '"}}',
                // url: "http://59.110.52.133:8000/clinicalpath/queryItemsTime/?q=%7B%22PID%22:%22"+cliPath.patientInfo.ID+"%22%7D",
                // beforeSend: function(request) {
                //     request.setRequestHeader("apikey", $rootScope.Authenapikey);
                //     request.setRequestHeader("token", $rootScope.userToken);
                // },
                // async: false,
            ...
            var queryItemP = $.ajax({
                url: 'http://59.110.52.133:3010/api/transfer?q={"path":"/clinicalpath/queryItem","params":{"PID":"' +
                        cliPath.patientInfo.ID + '"}}',
                // url: "http://59.110.52.133:8000/clinicalpath/queryItem/?q=%7B%22PID%22:%22"+cliPath.patientInfo.ID+"%22%7D",
                // async: false,
                // beforeSend: function(request) {
                //     request.setRequestHeader("apikey", $rootScope.Authenapikey);
                //     request.setRequestHeader("token", $rootScope.userToken);
